<template>
	<div class="arena-container">
		...
		<div v-if="showToast" id="toast" class="show">
			<div id="desc">
		...
		<div class="boss-container">
			...
			<div class="attack-container">
				<button class="cta-button" @click="runAttackAction">
					💥 Attack {{ boss.name }}
				</button>
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const fetchBoss = async () => {
			const bossTxn = await gameContract.value.getBigBoss();
			console.log("Boss:", bossTxn);

			boss.value = transformCharacterData(bossTxn);
		};

		const runAttackAction = async () => {
			try {
				if (gameContract) {
					attackState.value = "attacking";
					console.log("Attacking boss...");
					const attackTxn = await gameContract.value.attackBoss();
					await attackTxn.wait();
					console.log("attackTxn:", attackTxn);
					attackState.value = "hit";
		...
		const onAttackComplete = (newBossHp: number, newPlayerHp: number) => {
			...
			boss.value.hp = newBossHp;
			emit("on-character-attacked", newPlayerHp);
		};
		...
		watchEffect(async () => {
			...
			fetchBoss();
			gameContract.value.on("AttackComplete", onAttackComplete);
		});
